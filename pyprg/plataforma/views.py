...
def pacientes(request):
    ...
    if request.method == 'GET':
        # buscar os pacientes já cadastrados por este nutricionista
        ...
        if (len(nome.strip()) == 0) or (len(sexo.strip()) == 0) or (len(idade.strip()) == 0) or (len(email.strip()) == 0) or (len(telefone.strip()) == 0): # noqa
        ...
        try:
            paciente = Pacientes(
                ...
                email=email,
                telefone=telefone,
                # a nutricionista será o user que esta logado e que esta fazendo o cadastro
                nutri=request.user)
            paciente.save()
...
def dados_paciente(request, id):
    ...
    # verifiar se o paciente pertence ao nutricionata
    ...
    if (len(peso.strip()) == 0) or (len(altura.strip()) == 0) or (len(gordura.strip()) == 0) or (len(musculo.strip()) == 0) or (len(hdl.strip()) == 0) or (len(ldl.strip()) == 0) or (len(colesterol_total.strip()) == 0) or (len(triglicerídios.strip()) == 0): # noqa
        messages.add_message(
            request, constants.ERROR, 'Preencha todos os campos')
        return redirect('/dados_paciente/')

    if not peso.isnumeric() or altura.isnumeric() or gordura.isnumeric() or musculo.isnumeric() or hdl.isnumeric() or ldl.isnumeric() or colesterol_total.isnumeric() or triglicerídios.isnumeric(): # noqa
        messages.add_message(request, constants.ERROR, 'Digite uma idade válida')
        return redirect('/dados_paciente/')
...
def plano_alimentar(request, id):
    paciente = get_object_or_404(Pacientes, id=id)
    # verificar se o paciente pertence a nutricionista
    if not paciente.nutri == request.user:
        messages.add_message(request, constants.ERROR, 'Esse paciente não é seu')
...
def opcao(request, id_paciente):
    if request.method == "POST":
        ...
        # atenção que aqui temos FILES e não POST
        ...
        print(id_refeicao, imagem, descricao)
